    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px 8px', borderBottom: `1px solid ${border.subtle}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#eab308" strokeWidth="2.5" strokeLinecap="round">
          ...
          <span style={{ fontSize: 12, color: '#a1a1aa' }}>
            ...
            <span style={{ color: '#fafafa', fontWeight: 500 }}>"{clean}"</span>
            {' '}originals on SoundCloud:
          </span>
        </div>
        <button
          onClick={onClose}
          style={{ background: 'none', border: 'none', color: '#52525b', cursor: 'pointer', padding: 4, display: 'flex', transition: 'color 0.1s' }}
      ...
      <div style={{ padding: '8px 14px 12px' }}>
        {state === 'loading' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 0', color: '#52525b', fontSize: 12 }}>
        ...
        {state === 'error' && (
          <p style={{ fontSize: 12, color: '#ef4444', padding: '8px 0' }}>Could not load alternatives</p>
        )}

        {state === 'done' && results.length === 0 && (
          <p style={{ fontSize: 12, color: '#52525b', padding: '8px 0' }}>No alternatives found</p>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {results.map(track => {
              ...
              return (
                ...
                >
                  {/* Artwork */}
                  <div style={{ width: 34, height: 34, borderRadius: 5, flexShrink: 0, overflow: 'hidden', background: '#27272a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {track.artwork_url ? (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 13, fontWeight: 500, color: '#fafafa', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {track.title}
                      </span>
                      {!modified && (
                        <span style={{ fontSize: 10, fontWeight: 600, color: '#22c55e', background: 'rgba(34,197,94,0.1)', borderRadius: 3, padding: '1px 5px', whiteSpace: 'nowrap', flexShrink: 0 }}>
                          ORIGINAL
                        </span>
                      )}
                    </div>
                    <span style={{ fontSize: 11, color: '#71717a' }}>{track.artist}</span>
                  </div>
                  ...
                  {/* Arrow */}
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={isHov ? '#f97316' : '#3f3f46'} strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0, transition: 'stroke 0.12s' }}>
